...
def detect_top_chrome(gray_img, width, height):
    ...
    if top_offset < 30:
        ...
        for i in range(10, len(gradient) - 10):
            if abs(gradient[i]) > 5 and abs(gradient[i]) > abs(gradient[i - 1]) and abs(gradient[i]) > abs(
                    gradient[i + 1]):
...
def detect_bookmarks(gray_img, top_offset, width, height):
    ...
    if bookmark_region_end > bookmark_region_start + 10:
        ...
        lines_bookmark = cv2.HoughLinesP(edges_bookmark, 1, np.pi / 180, threshold=50, minLineLength=width * 0.4,
                                         maxLineGap=20)
        ...
        if bookmark_evidence >= 5:
            ...
            if bookmark_offset > 40:
                bookmark_offset = 35
            print(
                f"[INFO] Bookmarks bar detected with height: {bookmark_offset}px (evidence score: {bookmark_evidence})")
        else:
...
def detect_bottom_taskbar(gray_img, width, height):
    ...
    lines_bottom = cv2.HoughLinesP(edges_bottom, 1, np.pi / 180, threshold=100, minLineLength=width * 0.25,
                                   maxLineGap=20)
