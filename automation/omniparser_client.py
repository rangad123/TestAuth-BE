...
# Define standard browser UI dimensions that will be used for all testing environments
# These values represent our standardized testing environment
#STANDARD_MODE_TOP_OFFSET = 120  # Standard height for address bar and tabs
STANDARD_MODE_BOTTOM_OFFSET = 0  # Standard bottom offset (usually 0)
...
def detect_browser_ui(img):
    ...
    lines_bookmark = cv2.HoughLinesP(edges_bookmark, 1, np.pi / 180, threshold=100, minLineLength=width * 0.3,
                                     maxLineGap=10)
    ...
    lines_bottom = cv2.HoughLinesP(edges_bottom, 1, np.pi / 180, threshold=100, minLineLength=width * 0.3,
                                   maxLineGap=10)
...
def create_coordinate_sets(content_coords, original_offset, has_bookmark_bar, bookmark_bar_height):
    """
    Create coordinates normalized for Guest mode (no bookmarks).

    Args:
        content_coords: Element coordinates in the cropped image
        original_offset: The top offset detected in the original screenshot
        has_bookmark_bar: Whether bookmark bar was detected
        bookmark_bar_height: Height of the bookmark bar if present

    Returns:
        Dictionary with both original and normalized (for guest mode) coordinates
    """
    # Original screenshot coordinates - add back the original browser UI offset
    original_coords = [
        content_coords[0],
        content_coords[1] + original_offset,
        content_coords[2],
        content_coords[3] + original_offset
    ]

    # Calculate normalized coordinates for Guest mode (no bookmarks)
    address_bar_height = original_offset - (bookmark_bar_height if has_bookmark_bar else 0)

    # Normalized coordinates for Guest mode (without bookmarks)
    guest_mode_coords = [
        content_coords[0],
        content_coords[1] + address_bar_height,  # Only add the address bar height, not bookmark height
        content_coords[2],
        content_coords[3] + address_bar_height
    ]

    # Calculate click point based on guest mode coordinates
    center_x = (guest_mode_coords[0] + guest_mode_coords[2]) // 2
    center_y = (guest_mode_coords[1] + guest_mode_coords[3]) // 2

    return {
        "original_coords": original_coords,
        "guest_mode_coords": guest_mode_coords,
        "address_bar_height": address_bar_height,
...
def Write_to_omniparser(screenshot_path):
    ...
    try:
        ...
        print("=" * 50)
        print(f"[DEBUG] Image Dimensions: width={original_width}, height={original_height}")
        print(f"[DEBUG] Top Offset (Address + Bookmark Bar): {top_offset}")
        print(f"[DEBUG] Bookmark Bar Height: {bookmark_bar_height}")
        print(f"[DEBUG] Has Bookmark Bar: {has_bookmark_bar}")
        print(f"[DEBUG] Bottom Offset: {bottom_offset}")
        print("=" * 50)
        ...
        # Draw red line at top_offset (which includes both address bar and bookmarks if present)
        cv2.line(annotated_img, (0, top_offset), (original_width, top_offset), (0, 0, 255), 2)
        cv2.putText(annotated_img, "Detected Browser UI", (10, top_offset - 5),
        ...
        if has_bookmark_bar and bookmark_bar_height > 0:
            address_bar_boundary = top_offset - bookmark_bar_height
            cv2.line(annotated_img, (0, address_bar_boundary), (original_width, address_bar_boundary), (0, 255, 255), 2)
            cv2.putText(annotated_img, "Address Bar Bottom", (10, address_bar_boundary - 5),
        ...
        print(f"[INFO] Coordinate mapping:")
        print(f"       - Detected top UI height: {top_offset}px")
        print(f"       - Has bookmark bar: {'Yes' if has_bookmark_bar else 'No'}")
        #print(f"       - Standard mode fixed top UI height: {STANDARD_MODE_TOP_OFFSET}px")
        ...
        for match in matches:
            ...
            # Create coordinate sets with actual detected values
            coordinates = create_coordinate_sets(
                content_coords,
                original_offset=top_offset,
                has_bookmark_bar=has_bookmark_bar,
                bookmark_bar_height=bookmark_bar_height
            )
            ...
            element = {
                ...
                "original_screen_coordinates": coordinates["original_coords"],
                "guest_mode_coordinates": coordinates["guest_mode_coords"],  # Add normalized coordinates
                "browser_ui": {
                    "address_bar_height": coordinates["address_bar_height"],
                    "has_bookmark_bar": coordinates["has_bookmark_bar"],
                    "bookmark_bar_height": coordinates["bookmark_bar_height"]
                },
                "click_point": coordinates["click_point"]  # This is now based on guest mode coordinates
            }
        ...
        for element in elements:
            # Draw original coordinates in red
            original_x1 = element["original_screen_coordinates"][0]
            original_y1 = element["original_screen_coordinates"][1]
            original_x2 = element["original_screen_coordinates"][2]
            original_y2 = element["original_screen_coordinates"][3]

            cv2.rectangle(mapping_img, (original_x1, original_y1), (original_x2, original_y2), (0, 0, 255), 2)
            cv2.putText(mapping_img, f"Original: {element['name'][:10]}...",
                        (original_x1, original_y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)

            # Draw guest mode coordinates in green
            guest_x1 = element["guest_mode_coordinates"][0]
            guest_y1 = element["guest_mode_coordinates"][1]
            guest_x2 = element["guest_mode_coordinates"][2]
            guest_y2 = element["guest_mode_coordinates"][3]

            cv2.rectangle(mapping_img, (guest_x1, guest_y1), (guest_x2, guest_y2), (0, 255, 0), 2)
            cv2.putText(mapping_img, f"Guest Mode: {element['name'][:10]}...",
                        (guest_x1, guest_y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)

            # Draw click points
            cv2.circle(mapping_img, element["click_point"], 5, (255, 255, 0), -1)

            # # Draw standard coordinates in green
            # std_x1 = element["standard_coordinates"][0]
            # std_y1 = element["standard_coordinates"][1]
            # std_x2 = element["standard_coordinates"][2]
            # std_y2 = element["standard_coordinates"][3]
            #
            # # Draw rectangle for standard coords (green)
            # cv2.rectangle(mapping_img, (std_x1, std_y1), (std_x2, std_y2), (0, 255, 0), 2)
            # cv2.putText(mapping_img, f"Standard: {element['name'][:10]}...",
            #             (std_x1, std_y1 - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 1)
